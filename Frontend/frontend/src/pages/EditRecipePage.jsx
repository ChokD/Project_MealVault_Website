...
function EditRecipePage() {
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.message || 'ไม่พบสูตรอาหารนี้');
        }
        ...
        setInitialTotalTime(data.total_time_minutes ?? '');
        setServings(data.servings ?? '');
        setIngredients(Array.isArray(data.ingredients) && data.ingredients.length > 0
          ? data.ingredients.map(item => ({
              name: item.name || item.ingredient || '',
              amount: item.amount || item.measure || ''
            }))
          : [emptyIngredient]);
        setSteps(Array.isArray(data.steps) && data.steps.length > 0
          ? data.steps.map(item => ({ detail: item.detail || item.step || '' }))
          : [emptyStep]);
  ...
  const removeIngredient = (index) => setIngredients(prev => prev.filter((_, idx) => idx !== index));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('steps', JSON.stringify(filteredSteps.map((item, index) => ({ order: index + 1, detail: item.detail }))));
      if (imageFile) {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || 'ไม่สามารถแก้ไขสูตรอาหารได้');
      }

      setSuccess('แก้ไขสูตรอาหารสำเร็จ!');
      setTimeout(() => {
        navigate(`/menus/${recipeId}`);
      }, 1200);
    } catch (err) {
        ...
        <div className="container mx-auto px-6 sm:px-8 max-w-5xl">
          <div className="bg-white shadow-xl rounded-2xl p-8">
            <div className="flex items-start justify-between mb-8">
              <div>
                <h1 className="text-3xl font-bold text-gray-800 mb-2">แก้ไขสูตรอาหาร</h1>
                <p className="text-gray-500">ปรับปรุงสูตรของคุณให้สมบูรณ์ยิ่งขึ้น</p>
              </div>
              <button
                type="button"
                onClick={() => navigate(`/menus/${recipeId}`)}
            ...
            {currentImage && (
              <div className="mb-6">
                ...
                <img src={currentImage} alt="current" className="w-full max-h-64 object-cover rounded-xl border" />
              </div>
            )}
                ...
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-yellow-700">
                      ระบบตรวจพบความคล้ายคลึง {Math.round((plagiarismWarning.similarityScore || 0) * 100)}%
                    </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">รูปภาพประกอบ (ถ้าต้องการเปลี่ยน)</label>
                    <input
                      type="file"
                      accept="image/*"
              ...
              {success && <div className="bg-emerald-100 text-emerald-700 px-4 py-2 rounded-lg">{success}</div>}
